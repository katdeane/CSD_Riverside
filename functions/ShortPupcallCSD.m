function ShortPupcallCSD(homedir,subject,cbar)
...
pupCondList = Cond.ShortCall{subID};
...
for iMeas = 1:length(pupCondList)
    ...
    ylabel('Time [s]')
    % ylabel
    ylabel('Cortical depth [Channels 50 µm apart]')
    % title
    ...
    PupTimesCSD = ShortPupTimes .* 1000; % match axes

    % sanity checks 
    % nexttile
    % plot(t,ymod,'k'); xlabel('Seconds'); ylabel('PupCall');
    % xlim([0 size(CSD,2)/1000])
    % xline(ShortPupTimes(:,1),'b')
    % xline(ShortPupTimes(:,2),'r')
    % colormap jet; clim(cbar); colorbar
    % 
    % nexttile
    % imagesc(CSD)
    % xline(ShortPupTimes(:,1).*1000,'b')
    % xline(ShortPupTimes(:,2).*1000,'r')

    callList = 1:10; % all calls
