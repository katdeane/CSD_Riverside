function runCwtCsd_LFP(Group,params,homedir,freqlist)

% Input:    group name to match *Data.mat in \Dynamic_CSD\DATA, parameters
%           set for CWT analysis, home directory
...
for iAn = 1:entries
    ...
    for iFreq = 1:length(freqlist)
        ...
        for iMeas = 1:nummeas
            ...
            curLFP = Data(thisFreq).SngTrl_LFP{iMeas}; %chan x time x trial
           ...
           for iLay = 1:length(params.layers)
               ...
               for itrial = 1:size(curLFP,3)
                ...
                for iChan = 1:length(curChan)
                    ...
                    lfpChan = squeeze(curLFP(curChan(iChan),timeaxis,itrial));
                    
                    if isnan(sum(lfpChan))
                        continue
                    end
                    % Limit the cwt frequency limits
                    params.frequencyLimits(1) = max(params.frequencyLimits(1),...
                        cwtfreqbounds(numel(lfpChan),params.sampleRate,...
                        'TimeBandWidth',params.timeBandWidth));
                    
                    [WT,F] = cwt(lfpChan,params.sampleRate, ...
        ...
        save([Aname '_' freqlist{iFreq} '_WT_LFP.mat'],'wtTable');
