function gapASSRStats_3grp(homedir,Groups)
...
grp3sgl = readtable([Groups{3} '_gapASSR_AVRECPeak.csv']);
...
grp3avg = readtable([Groups{3} '_gapASSR_AVG_AVRECPeak.csv']);
...
grp3sgl = grp3sgl(grp3sgl.ClickFreq == gapwidth,:);
grp1avg = grp1avg(grp1avg.ClickFreq == gapwidth,:);
grp2avg = grp2avg(grp2avg.ClickFreq == gapwidth,:);
grp3avg = grp3avg(grp3avg.ClickFreq == gapwidth,:);
...
PeakStats.Properties.VariableNames = ["Comp", "Layer", "stat", "SPA",...
...
for iLay = 1:length(layers)
    ...
    avg3lay =  avg3lay(avg3lay.OrderofClick >= 4,:);
    sgl1lay =  sgl1lay(sgl1lay.OrderofClick >= 4,:);
    sgl2lay =  sgl2lay(sgl2lay.OrderofClick >= 4,:);
    sgl3lay =  sgl3lay(sgl3lay.OrderofClick >= 4,:);
