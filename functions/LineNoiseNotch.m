function filteredSignal = LineNoiseNotch(sngtrlLFP)
...
filteredSignal = nan(size(sngtrlLFP{:},1),size(sngtrlLFP{:},2),size(sngtrlLFP{:},3));
for ichan = 1:size(sngtrlLFP{:},1)
    for itrial = 1:size(sngtrlLFP{:},3)
        filteredSignal(ichan,:,itrial) = filtfilt(b, a, sngtrlLFP{:}(ichan,:,itrial));
    end
end
...
% filtered FFT
meanFiltSig = nanmean(filteredSignal,3);
