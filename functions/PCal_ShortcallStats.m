function PCal_ShortcallStats(homedir,Groups,call_list)
...
for iLay = 1:length(layers)
    ...
    grp1size = length(unique(sgl1lay.Animal));
    grp1name = unique(sgl1lay.Animal);
    grp2size = length(unique(sgl2lay.Animal));
    grp2name = unique(sgl2lay.Animal);

    avg1stk  = nan(grp1size,max(avg1lay.OrderofClick)); % subject x peak
    sgl1stk  = nan(grp1size,max(sgl1lay.OrderofClick),50); % maximum trials is 50
    avg2stk  = nan(grp2size,max(avg1lay.OrderofClick));
    sgl2stk  = nan(grp2size,max(sgl2lay.OrderofClick),50);
    ...
    cd PeakPlots
    h = gcf;
    savefig(h,[Groups{1} 'v' Groups{2} '_' layers{iLay} '_ShortCall']);
    exportgraphics(h,[Groups{1} 'v' Groups{2} '_' layers{iLay} '_ShortCall.pdf'])
    close(h)
...
writetable(PeakStats,[Groups{1} 'v' Groups{2} '_ShortCall_Stats.csv']);
cd(homedir)