function [peakout,latencyout,rmsout] = shortpupcall_peaks(datain, call_list,BL)
...
if ~exist('call_list','var')
    call_list  = 1:10; % Hz of stim presentation
end
...
ShortPupTimes = ShortPupTimes .* 1000; % match axes for CSD
...
for iCall = 1:numreps
    det_on(iCall) = ShortPupTimes(call_list(iCall),1);
end
det_off = det_on + 200;
