function TracesOLDvYNGfig(homedir, Condition)
...
for iStim = 1:length(thisstim)
    ...
    for iLay = 1:length(layers)
        ...
        % new 70 dB
        openfig(['YNG_CSDTraces_' Condition(1:end-2) '70_' layers{iLay} '.fig']);
        pause(1)
        h = gca;
        ...
        % old 70 dB
        openfig(['OLD_CSDTraces_' Condition(1:end-2) '70_' layers{iLay} '.fig']);
        pause(1)
        h = gca;
        ...
        % old 80 dB
        openfig(['OLD_CSDTraces_' Condition(1:end-2) '80_' layers{iLay} '.fig']);
        pause(1)
        h = gca;
        ...
        if matches(layers{iLay},'All')
            legend('', 'Young 70 dB', '', 'Old 70 dB', '', 'Old 80 dB')
    ...
    exportgraphics(targetfig,['OLDvYNG_TrioTraces_' Condition(1:end-2) '_' thisstim{iStim} '.png'])
    close;
end
cd(homedir)